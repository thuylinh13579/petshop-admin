...
import { UPDATE_PRODUCT_ENDPOINT } from "@/utils/constants/endpoints"
...
const UpdateProduct = (props: IUpdateProductProps) => {
    const { isOpenEdit, handleClose, product } = props
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            const response = await axios.put(UPDATE_PRODUCT_ENDPOINT(product?._id), data);
            if(response.status == 200){
                handleClose(false);
                dispatch(setError({ status: 'success', message: 'Update product successfully!' }));
            } else {
                dispatch(setError({ status: 'danger', message: 'Update product failed!' }));
            }
        }
                                ...
                                <div className="mt-3">
                                    <form onSubmit={onSubmit} data-parsley-validate>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            <div>
                                                <label htmlFor="originalPrice" className="block text-sm font-medium text-gray-700">Giá gốc
                                                    (VNĐ)</label>
                                                <input onChange={handleChange} value={formData.originalPrice} type="number" id="originalPrice" name="originalPrice"
                                                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
                                            </div>
                                            <div>
                                                <label htmlFor="discountedPrice" className="block text-sm font-medium text-gray-700">Giá mới
                                                    (VNĐ)</label>
                                                <input onChange={handleChange} value={formData.discountedPrice} type="number" id="discountedPrice" name="discountedPrice"
                                                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
                                            </div>
                                        </div>
                                        <div className="mb-4">
                                            <label htmlFor="quantity" className="block text-sm font-medium text-gray-700">Số
                                                lượng tồn kho</label>
                                            <input onChange={handleChange} value={formData.quantity} type="number" id="quantity" name="quantity"
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
                                        </div>
                                        <div className="mb-4">
                                            <label htmlFor="category" className="block text-sm font-medium text-gray-700">
                                                Danh mục</label>
                                            <select id="category" name="category" value={formData.category} onChange={handleSelectChange}
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <input onChange={handleChange} value={formData.brand} type="text" id="brand" name="brand"
                                                className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
                                        </div>
                                        <div className="mb-4">
